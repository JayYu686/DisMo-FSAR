...
class Split_few_shot():
    ...
    def __init__(self, folder, split_dataset='train', dataset="Ssv2_few_shot"):
        ...
        def _parse_class_id(prefix_token):
            """
            Parse class id from tokens like train12 / val3 / test7 robustly.
            """
            matched = re.search(r"(\d+)$", str(prefix_token))
            if matched is None:
                raise ValueError(f"Cannot parse class id from token: {prefix_token}")
            return int(matched.group(1))

        if dataset_normalized == 'ssv2_few_shot':

            for class_folder in folder:
                
                paths = class_folder.strip().split('/')[-1]

                class_id = _parse_class_id(class_folder.strip().split('/')[0]) # class_folders.index(class_folder)
...
class Ssv2_few_shot(BaseVideoDataset):
    ...
    def _get_dataset_list_name(self):
        ...
        """
        # Benchmark-compatible behavior:
        # train always uses train_few_shot.txt, while evaluation uses
        # test_few_shot.txt regardless of TEST.TEST_SET. This matches the
        # convention used by the reference FSAR codebases in this workspace.
        split_tag = "train" if self.split == "train" else "test"
        name = "{}_few_shot.txt".format(split_tag)
        ...
        for retry in range(5):
            try:
                ...
                if retry<4:
                    ...
                    raise ValueError(
                        "Failed to load or parse data list {}.".format(
                            os.path.join(self.anno_dir, dataset_list_name)
                        )
                    )
    ...
    def __len__(self):
        if hasattr(self.cfg.TRAIN, "META_BATCH") and self.split == 'train' and self.cfg.TRAIN.META_BATCH:
            if hasattr(self.cfg.TRAIN, "NUM_SAMPLES"):
                return self.cfg.TRAIN.NUM_SAMPLES
            if hasattr(self.cfg.TRAIN, "NUM_TRAIN_TASKS"):
    ...
    def _config_transform(self):
        ...
        if self.split == 'train' and not self.cfg.PRETRAIN.ENABLE:
            use_random_flip = (
                hasattr(self.cfg.AUGMENTATION, "RANDOM_FLIP") and
                self.cfg.AUGMENTATION.RANDOM_FLIP
            )
            ...
            if use_random_flip:
                std_transform_list_query.insert(1, transforms.RandomHorizontalFlipVideo())
...
@DATASET_REGISTRY.register()
class Kinetics_few_shot(Ssv2_few_shot):
    """Few-shot episodic dataset reader for Kinetics split format."""


@DATASET_REGISTRY.register()
class Hmdb_few_shot(Ssv2_few_shot):
    """Few-shot episodic dataset reader for HMDB split format."""


@DATASET_REGISTRY.register()
class Ucf_few_shot(Ssv2_few_shot):
    """Few-shot episodic dataset reader for UCF split format."""
