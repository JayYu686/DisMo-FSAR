...
_TRANSIENT_EVAL_BUFFER_SUFFIXES = (
    "test_text_features",
    "test_global_text_features",
    "test_entity_text_features",
    "test_phase_text_features",
)


def _drop_transient_eval_buffers(state_dict):
    """Remove split-specific semantic caches that should not affect checkpoint loading."""
    if state_dict is None:
        return
    stale_keys = [
        key for key in list(state_dict.keys())
        if any(key.endswith(suffix) for suffix in _TRANSIENT_EVAL_BUFFER_SUFFIXES)
    ]
...
def load_checkpoint(
    ...
    _drop_transient_eval_buffers(checkpoint["model_state"])
    if pre_process:
    ...
    if "model_ema_state" in checkpoint.keys():
        _drop_transient_eval_buffers(checkpoint["model_ema_state"])
    if inflation:
...
def load_train_checkpoint(cfg, model, model_ema, optimizer, model_bucket=None):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        if hasattr(cfg.TRAIN, "SAVE_BEST_ONLY") and cfg.TRAIN.SAVE_BEST_ONLY:
            best_name = getattr(cfg.TRAIN, "BEST_CHECKPOINT_NAME", "checkpoint_best.pyth")
            best_path = os.path.join(get_checkpoint_dir(cfg.OUTPUT_DIR), best_name)
            if os.path.exists(best_path):
                last_checkpoint = best_path
                logger.info("Load from best checkpoint (best-only), {}.".format(last_checkpoint))
            else:
                stem, ext = os.path.splitext(best_name)
                if ext == "":
                    ext = ".pyth"
                candidates = sorted(
                    glob.glob(os.path.join(get_checkpoint_dir(cfg.OUTPUT_DIR), "{}_*{}".format(stem, ext)))
                )
                if len(candidates) > 0:
                    last_checkpoint = candidates[-1]
                    logger.info(
                        "Load from best checkpoint pattern (best-only), {}.".format(last_checkpoint)
                    )
