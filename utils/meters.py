...
def _err_to_acc(err_value):
    return 100.0 - float(err_value)


def _normalize_metric_key_and_value(key, value):
    """Convert *_err metrics into *_acc for logging readability."""
    if "top1_err" in key:
        return key.replace("top1_err", "top1_acc"), _err_to_acc(value)
    if "top5_err" in key:
        return key.replace("top5_err", "top5_acc"), _err_to_acc(value)
    return key, value
...
class TrainMeter(object):
    ...
    def update_stats(self, top1_err, top5_err, loss, lr, mb_size, **kwargs):
        ...
        for k,v in kwargs.items():
            ...
            metric_key, metric_val = _normalize_metric_key_and_value(k, v)
            self.opts[metric_key].add_value(metric_val)
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if not self._cfg.PRETRAIN.ENABLE and not self._cfg.LOCALIZATION.ENABLE:
            stats["top1_acc"] = _err_to_acc(self.mb_top1_err.get_win_median())
            stats["top5_acc"] = _err_to_acc(self.mb_top5_err.get_win_median())
        logging.log_json_stats(stats)
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if not self._cfg.PRETRAIN.ENABLE:
            ...
            stats["top1_acc"] = _err_to_acc(top1_err)
            stats["top5_acc"] = _err_to_acc(top5_err)
            stats["loss"] = avg_loss
        logging.log_json_stats(stats)
...
class ValMeter(object):
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        top1_acc = _err_to_acc(top1_err)
        top5_acc = _err_to_acc(top5_err)
