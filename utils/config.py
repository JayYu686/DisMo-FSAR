...
class Config(object):
    ...
    def _merge_cfg_from_command(self, args, cfg):
        ...
        keys = args.opts[0::2]
        vals = [self._decode_override_value(v) for v in args.opts[1::2]]
    ...
    @staticmethod
    def _decode_override_value(raw_val):
        """
        Decode command-line override values into native python types.
        Examples:
            "False" -> False
            "8" -> 8
            "0.001" -> 0.001
        ...
        if raw_val == "":
            return raw_val
        try:
            decoded = yaml.safe_load(raw_val)
        except Exception:
            return raw_val
        # Keep non-empty raw string if yaml cannot infer a concrete value.
        if decoded is None and raw_val.strip().lower() not in {"none", "null", "~"}:
            return raw_val
        return decoded
