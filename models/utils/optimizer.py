...
def _set_cfg_value(section, field, value):
    setattr(section, field, value)
    if hasattr(section, "cfg_dict") and isinstance(section.cfg_dict, dict):
        section.cfg_dict[field] = value


def sync_solver_optimizer_cfg(cfg):
    """
    Keep OPTIMIZER settings consistent with SOLVER for training code paths that
    schedule LR from SOLVER but construct optimizers from OPTIMIZER.
    """
    if not hasattr(cfg, "SOLVER") or not hasattr(cfg, "OPTIMIZER"):
        return {}
    ...
    changes = {}
    for field in fields:
        if not hasattr(cfg.SOLVER, field):
            continue
        solver_val = getattr(cfg.SOLVER, field)
        opt_val = getattr(cfg.OPTIMIZER, field, None)
        if opt_val != solver_val:
            _set_cfg_value(cfg.OPTIMIZER, field, solver_val)
            changes[field] = (opt_val, solver_val)

    if changes and hasattr(cfg, "cfg_dict") and isinstance(cfg.cfg_dict, dict):
        cfg.cfg_dict.setdefault("OPTIMIZER", {})
        for field, (_, new_val) in changes.items():
            cfg.cfg_dict["OPTIMIZER"][field] = new_val
